...
def find_nearest_idx(array, value):
    ...
    return idx
# %%
bnc_in = pd.read_csv("data/scope_13.csv", header=1)
bnc_out = pd.read_csv("data/scope_12.csv", header=1)
...
# %%
plt.plot(in_t+0.0000025, in_v)
plt.plot(out_t, out_v)
# %%
# f = np.logspace(np.log10(100e3), np.log10(1.5e6), in_t.shape[0])
f = np.logspace(np.log10(50e3), np.log10(800e3), in_t.shape[0])
# %%
in_h = hilbert(in_v)
out_h = hilbert(out_v)
...
plt.suptitle("BNC Probe Bode Plot")

axs[0].set_xscale('log')
axs[0].set_xlim(60e3, 780e3)

axs[0].plot(f, sosfilt(sos, 20 * np.log10(out_amp/0.5)), label='filtered')
axs[0].plot(f, 20 * np.log10(out_amp/0.5), alpha=0.5, label='raw')
axs[0].axhline(-3, linestyle='--', c='r', label="-3db")
axs[0].axvline(267e3, c='k', label="Measured $f_c$")
axs[0].legend()

pad_amt = 500
phase = np.mod(out_phase-in_phase, 2*np.pi) * 180 / np.pi - 300
